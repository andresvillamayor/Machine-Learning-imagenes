...
mascara = os.path.join(image_dir,"mascara_gas.png")
mascara2 = os.path.join(image_dir, "stormtrooper-ojos-transparentes.png")

# ver si el problema de los puntos de los ojos no sea por la ubicacion de los
# mismos dentro de el filtro
...
KEYPOINT_X_PORCENTAJE = 0.00
KEYPOINT_Y_PORCENTAJE = 0.00

#Activacion de Filtros
FILTRO_ACTIVADO = False
FILTRO_ACTUAL = 0
PRINT_DEBUG = False
#valor inicial del Frame
FRAME = 0
# Gestos de la imagen
CLASES_GESTOS = {
    0:'fondo',   # fondo normal
    1:'gestoA',  # puño cerrado
    2:'gestoB',  # mano abierta
    3:'gestoC'   # mano cerrada indice levantado (numero 1)
}
while True:
    ret, img = cap.read()  # leer la webcam
    img = cv.flip(img, 1)  # flip horizontal para que sea un espejo
    if FILTRO_ACTIVADO:
        #Redimensionar la ventana utilizando la libreria imutils
